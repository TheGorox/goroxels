import { p as assign_nodes, q as should_intro, r as is_boolean_attribute, a as append, f as from_html, c as comment, b as delegate, d as delegated, k as event$1, s as set_text, t as text, l as from_svg } from "../chunks/CfJiiG60.js";
import { h as hydrating, a as set_hydrate_node, t as template_effect, f as get_first_child, W as hydrate_next, a1 as active_effect, bf as remove_effect_dom, d as hydrate_node, C as COMMENT_NODE, g as get_next_sibling, aP as hydration_mismatch, aJ as HYDRATION_ERROR, aB as create_element, bg as NAMESPACE_SVG, bh as NAMESPACE_MATHML, X as EFFECT_TRANSPARENT, bi as BLOCK_EFFECT, aF as REACTION_RAN, $ as effect, m as untrack, bj as TRANSITION_GLOBAL, ag as is_function, ak as queue_micro_task, M as noop, aA as without_reactive_context, a0 as render_effect, p as deep_read_state, bk as safe_not_equal, bl as listen_to_event_and_reset_event, T as current_batch, G as tick, _ as HYDRATION_START, aO as HYDRATION_END, bm as STALE_REACTION, an as HYDRATION_START_FAILED, a$ as has_own_property, bn as ELEMENT_PRESERVE_ATTRIBUTE_CASE, bo as ELEMENT_IS_INPUT, bp as ELEMENT_IS_NAMESPACED, a7 as proxy, l as user_effect, bq as onDestroy, I as state, o as get, z as set$1, br as effect_root, bs as derived, bt as writable, O as get$1, v as push$1, w as pop$1, J as user_derived, bu as to_array, x as child, j as sibling, y as reset, i as first_child, F as onMount, bv as remove_textarea_child, aw as next } from "../chunks/BpheISZ8.js";
import { p as prop, i as if_block, r as rest_props, b as bind_this, s as spread_props, a as setup_stores, d as store_get } from "../chunks/BlNctscH.js";
import { e as escape_html, t as to_style, c as clsx, b as to_class, d as attr, f as toast, s as snippet, g as set_style, h as attribute_effect, i as each, j as index, k as set_class, S as STYLE, l as clsx$1, a as set_attribute, C as CLASS, T as Toast } from "../chunks/BTO0BmSa.js";
...
class Renderer {
   ...
   */
  child(fn) {
    const child2 = new Renderer(this.global, this);
  ...
  boundary(props, children_fn) {
    ...
    try {
      const result = children_fn(child2);
      ...
      child2.#out.length = 0;
      child2.#boundary = null;
      if (result instanceof Promise) {
        if (this.global.mode === "sync") {
          await_invalid();
        }
        child2.promise = /** @type {Promise<unknown>} */
        result.then((transformed) => {
          set_ssr_context(parent_context);
          child2.#out.push(Renderer.#serialize_failed_boundary(transformed));
          failed_snippet(child2, transformed, noop);
          child2.#out.push(BLOCK_CLOSE);
        });
        child2.promise.catch(noop);
      } else {
        child2.#out.push(Renderer.#serialize_failed_boundary(result));
        failed_snippet(child2, result, noop);
        child2.#out.push(BLOCK_CLOSE);
      }
    }
  }
  ...
  option(attrs, body, css_hash, classes, styles, flags, is_rich) {
    ...
    const close = (renderer, value, { head, body: body2 }) => {
      ...
      if (head) {
        renderer.head((child2) => child2.push(head));
      }
    };
  ...
  subsume(other) {
    ...
    this.#out = other.#out.map((item, i) => {
      ...
      if (current2 instanceof Renderer && item instanceof Renderer) {
  ...
  *#collect_ondestroy() {
    ...
    for (const child2 of this.#out) {
      if (child2 instanceof Renderer && !child2.#is_component_body) {
  ...
  async #collect_hydratables() {
    const ctx2 = get_render_context().hydratable;
    for (const [_, key] of ctx2.unresolved_promises) {
      unresolved_hydratable(key, ctx2.lookup.get(key)?.stack ?? "<missing stack trace>");
    }
    for (const comparison of ctx2.comparisons) {
      await comparison;
    }
    return await this.#hydratable_block(ctx2);
  }
  /**
   * @template {Record<string, any>} Props
  ...
  static #open_render(mode, component, options2) {
    ...
    try {
      ...
      const context = { p: null, c: options2.context ?? null, r: renderer };
      set_ssr_context(context);
   ...
   */
  async #hydratable_block(ctx2) {
    if (ctx2.lookup.size === 0) {
      return null;
    }
    let entries = [];
    let has_promises = false;
    for (const [k, v] of ctx2.lookup) {
      if (v.promises) {
        has_promises = true;
...
function createRenderer(canvas2, core2) {
  ...
  return { init: init2, requestRender, render: render2 };
}
...
const generalStickerpackIcon = "" + new URL("../assets/generalStickerpack.CvxYxOT6.png", import.meta.url).href;
...
function Popover($$anchor, $$props) {
  ...
  event$1("mouseenter", div, cancelClose);
  event$1("mouseleave", div, scheduleClose);
  append($$anchor, div);
  pop$1();
}
...
function Tabs($$anchor, $$props) {
  ...
  let activeIndex = user_derived(() => $$props.tabs?.findIndex((tab) => tab.id === active()) ?? 0);
  let resolvedPanelProps = user_derived(() => {
    if (Array.isArray(panelProps())) {
      return panelProps()[get(activeIndex)] ?? {};
    }
    return panelProps() ?? {};
  });
  var fragment = root$c();
  ...
  Panel(node, spread_props(() => get(resolvedPanelProps), {
    children: ($$anchor2, $$slotProps) => {
      var fragment_1 = comment();
...
var root_30 = from_html(`<div><img/></div>`);
var root_31 = from_html(`<img class="svelte-191yngm"/>`);
var root_29 = from_html(`<div class="stickersHeader svelte-191yngm"></div> <hr/> <div class="currentStickers svelte-191yngm"></div>`, 1);
...
function Chat($$anchor, $$props) {
  ...
  let activeStickers = user_derived(() => core2?.stickersCache?.find((p) => p.title === get(activeStickerpack))?.stickers || []);
        ...
        children: ($$anchor3, $$slotProps) => {
          {
            ...
            const stickers = ($$anchor4) => {
              ...
              var div_29 = first_child(fragment_14);
              each(div_29, 21, () => core2.stickersCache, (stickerpack) => stickerpack.id, ($$anchor5, stickerpack) => {
                var div_30 = root_30();
                var img = child(div_30);
                set_style(img, "", {}, { height: "32px" });
                reset(div_30);
                template_effect(() => {
                  set_class(div_30, 1, `stickerpackContainer ${get(activeStickerpack) === get(stickerpack).title ? "active" : ""}`, "svelte-191yngm");
                  set_attribute(img, "src", get(stickerpack).iconUrl || generalStickerpackIcon);
                  set_attribute(img, "alt", get(stickerpack).title);
                  set_attribute(img, "title", get(stickerpack).title);
                });
                delegated("click", div_30, () => set$1(activeStickerpack, get(stickerpack).title, true));
                append($$anchor5, div_30);
              });
              reset(div_29);
              var hr_2 = sibling(div_29, 2);
              set_style(hr_2, "", {}, { margin: "5px 0" });
              var div_31 = sibling(hr_2, 2);
              each(div_31, 21, () => get(activeStickers), (sticker) => sticker.id, ($$anchor5, sticker) => {
                var img_1 = root_31();
                template_effect(() => {
                  set_attribute(img_1, "src", get(sticker).thumbUrl);
                  set_attribute(img_1, "alt", get(sticker).code);
                  set_attribute(img_1, "title", `:${get(sticker).code}:`);
                });
                append($$anchor5, img_1);
              });
              reset(div_31);
              event$1("wheel", div_29, handleHorizontalScroll);
  ...
  var node_1 = sibling(button, 2);
  {
    var consequent_1 = ($$anchor2) => {
      ...
      each(div_1, 21, () => get(normalizedOptions), index, ($$anchor3, option, index2) => {
        ...
        event$1("mouseenter", button_1, () => set$1(focusedIndex, index2, true));
...
function Row($$anchor, $$props) {
  let alignLeft = prop($$props, "alignLeft", 3, "left"), alignRight = prop($$props, "alignRight", 3, "left"), padding = prop($$props, "padding", 3, 3);
    ...
    var consequent_1 = ($$anchor2) => {
      ...
      template_effect(
        ($0) => {
          ...
          styles_2 = set_style(div_2, "", styles_2, $0);
        },
  ...
  reset(div);
  template_effect(() => styles = set_style(div, "", styles, { "--padding": padding() + "px" }));
  append($$anchor, div);
}
...
var root_8 = from_html(`<div class="createStickerpackPanel">Create sticker pack <!></div> <hr/> <div class="stickersPanel">Add sticker <!> <div class="stickers svelte-3qzl15"></div></div>`, 1);
                  ...
                  children: ($$anchor4, $$slotProps2) => {
                    ...
                    each(div_4, 21, () => get(currentStickers), (sticker) => sticker.id, ($$anchor5, sticker) => {
                      ...
                      template_effect(() => {
                        ...
                        set_attribute(img, "title", `:${get(sticker).code}:`);
                      });
  ...
  onMount(() => {
    ...
    const resize = () => {
      ...
      for (const canvas2 of canvases) {
        ...
        setTimeout(() => {
          const ctx2 = canvas2.getContext("2d");
          ctx2 && (ctx2.imageSmoothingEnabled = false);
...
//# sourceMappingURL=2.D1XGRghY.js.map
