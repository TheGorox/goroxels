...
var root_2$2 = from_html(`<button class="removeBtn svelte-1hretyp" type="button">✕</button>`);
var root_1$9 = from_html(`<img alt="Preview" class="previewImage svelte-1hretyp"/> <div class="overlay svelte-1hretyp"><span class="svelte-1hretyp">Заменить</span> <!></div>`, 1);
var root_3$1 = from_html(`<div class="iconContainer svelte-1hretyp"></div> <span class="svelte-1hretyp"> </span>`, 1);
var root$7 = from_html(`<input type="file" style="display: none;"/> <div role="button" tabindex="0"><!></div>`, 1);
function UploadHandler($$anchor, $$props) {
  ...
  let value = prop(
    ...
    3,
    null
    // Можно передать File, URL-строку или null
  ), uploadedText = prop(
    $$props,
    "uploadedText",
    3,
    null
    // Если задан текст, показываем его вместо дефолтного после загрузки
  ), showPreview = prop(
    $$props,
    "showPreview",
    3,
    true
    // Отображать ли загруженную картинку поверх блока
  ...
  user_effect(() => {
    if (value() instanceof File) {
      ...
      set$1(localPreviewUrl, url, true);
      return () => URL.revokeObjectURL(url);
    } else if (typeof value() === "string") {
      set$1(localPreviewUrl, value());
    } else {
      set$1(localPreviewUrl, null);
    }
  });
  ...
  var node = child(div);
  {
    var consequent_1 = ($$anchor2) => {
      ...
      var node_1 = sibling(child(div_1), 2);
      {
        ...
        if_block(node_1, ($$render) => {
          if ($$props.onClear) $$render(consequent);
        });
      }
      reset(div_1);
      template_effect(() => set_attribute(img_1, "src", get(localPreviewUrl)));
      append($$anchor2, fragment_1);
    };
    var alternate = ($$anchor2) => {
      var fragment_2 = root_3$1();
      var div_2 = first_child(fragment_2);
      html(div_2, () => icon, true);
      reset(div_2);
      var span = sibling(div_2, 2);
      var text2 = child(span, true);
      reset(span);
      template_effect(($0) => set_text(text2, $0), [
        () => value() && uploadedText() ? uploadedText() : $_()("uploadContainerText")
      ]);
      append($$anchor2, fragment_2);
    };
    if_block(node, ($$render) => {
      if (showPreview() && get(localPreviewUrl)) $$render(consequent_1);
      else $$render(alternate, -1);
    });
  }
  reset(div);
  template_effect(() => {
    ...
    classes = set_class(div, 1, "uploadContainer svelte-1hretyp", null, classes, {
      dragging: get(isDragging),
      "has-preview": showPreview() && !!get(localPreviewUrl)
    });
  });
...
//# sourceMappingURL=2.CXaFskn0.js.map
