...
function preparePalettes(config2, allColors) {
  ...
  for (let i = 0; i < allColors.length; i++) {
    config2.colorsBGRtoIdx.set(config2.colorsBGR[i], i);
  }
...
class Socket {
  ...
  sendPixels(pixels, flags, options2 = {}) {
    ...
    dv.setUint8(1, flags & CLIENT_ALLOWED_FLAGS);
    let offset = 2;
    for (let i = 0; i < pixelData.length; i += 3) {
      const x2 = pixelData[i];
      const y2 = pixelData[i + 1];
      const c = pixelData[i + 2];
      dv.setUint16(offset, x2);
      offset += 2;
      dv.setUint16(offset, y2);
      offset += 2;
...
function processPendingPixels() {
  ...
  for (const [key, expires] of expiresMap) {
    if (now2 >= expires) {
      ...
      core2.chunkManager.setPixels([x2, y2, oldColMap.get(key)], false);
...
class BrushTool {
  ...
  drawTick(e) {
    ...
    this.core.socket.sendPixels(pixelsWithColors);
  }
  isLongTap() {
    return Date.now() - this.downTime > 600;
  }
...
//# sourceMappingURL=2.BRZFgiQ4.js.map
