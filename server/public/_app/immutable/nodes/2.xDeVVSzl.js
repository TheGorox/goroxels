import { p as assign_nodes, q as should_intro, a as append, f as from_html, c as comment, b as delegate, d as delegated, k as event$1, s as set_text, t as text, l as from_svg } from "../chunks/CsnEtAjo.js";
...
const config$1 = proxy({
  ...
  colors: [],
  colorsBGR: [],
  colorsBGRtoIdx: /* @__PURE__ */ new Map(),
  colorsHex: [],
  shared: {}
});
function initConfig() {
...
function preparePalettes(config2, allColors) {
  ...
  for (let i2 = 0; i2 < allColors.length; i2++) {
    config2.colorsBGRtoIdx.set(config2.colorsBGR[i2], i2);
  }
...
const expiresMap = /* @__PURE__ */ new Map();
const oldColMap = /* @__PURE__ */ new Map();
function queuePixelRevert(x2, y2, oldColor) {
  const key = y2 * MULTIPLIER + x2;
  if (expiresMap.has(key)) {
    expiresMap.set(key, performance.now() + 3e3);
    return;
  }
  expiresMap.set(key, performance.now() + 3e3);
  oldColMap.set(key, oldColor);
}
function processPendingPixels() {
  if (expiresMap.size === 0) return;
  const core2 = useGameCore$1();
  if (!core2 || !core2.chunkManager) return;
  const now2 = performance.now();
  let needRender = false;
  for (const [key, expires] of expiresMap) {
    if (now2 >= expires) {
      const y2 = Math.floor(key / MULTIPLIER);
      const x2 = key % MULTIPLIER;
      core2.chunkManager.setChunkPixel(x2, y2, oldColMap.get(key));
      expiresMap.delete(key);
      oldColMap.delete(key);
      needRender = true;
    }
  }
  if (needRender) core2.renderer.needRender = true;
}
...
function createRenderer(canvas2, core2) {
  ...
  function render() {
    ...
    for (let i2 = 0; i2 < chunks.length; i2 += 2) {
      ctx2.globalAlpha = 1;
      const cx = chunks[i2];
      const cy = chunks[i2 + 1];
...
class EventEmitter {
  ...
  emit(eventName, ...args) {
    ...
    for (let i2 = 0; i2 < currentListeners.length; i2++) {
      currentListeners[i2].call(this, ...args);
    }
    return true;
  }
}
...
function getClosestAvailableLocale(refLocale) {
  ...
  for (let i2 = 0; i2 < relatedLocales.length; i2++) {
    const locale = relatedLocales[i2];
    if (hasLocaleDictionary(locale)) {
      return locale;
    }
  }
...
function getSubLocales(refLocale) {
  return refLocale.split("-").map((_, i2, arr) => arr.slice(0, i2 + 1).join("-")).reverse();
}
...
class Chunk {
  ...
  loadFromBuffer(buffer) {
    ...
    for (let i2 = 0; i2 < buffer.byteLength; i2++) {
      col = buffer[i2];
      isProtected = col & 128;
      isProtected && (this.pView[i2] = 4294901760);
      this.view[i2] = bgrPalette[col & 127];
    }
  }
  set(x2, y2, col) {
    const bgrPalette = this.core.config.colorsBGR;
    const i2 = x2 + y2 * this.width;
    this.view[i2] = bgrPalette[col];
  }
  get(x2, y2) {
...
function initChunkManager(core2) {
  ...
  function startLimboInterval() {
    setInterval(() => {
      ...
      chunkList = chunkList.reduce((acc, _, i2, src) => (
        // reshape flat chunk list into pairs of [cx, cy]
        i2 % 2 === 0 ? [...acc, src.slice(i2, i2 + 2)] : acc
      ), []);
  ...
  async function checkChunks2() {
    ...
    try {
      ...
      try {
        ...
        for (let i2 = 0; i2 < validateQueueChunks.length; i2++) {
          const isHashValid = !!result[i2];
          const chunkKey = getChunkKey(...validateQueueChunks[i2]);
          if (isHashValid) {
  ...
  function setPixels(pixelsArr) {
    for (let i2 = 0; i2 < pixelsArr.length; i2 += 3) {
      const x2 = pixelsArr[i2];
      const y2 = pixelsArr[i2 + 1];
      const colId = pixelsArr[i2 + 2];
...
class WebGLShaderEffect {
  ...
  #cacheUniforms() {
    ...
    for (let i2 = 0; i2 < numUniforms; i2++) {
      const info = gl.getActiveUniform(this.program, i2);
...
const shapes = {
  ...
  line: function(x2, y2, x22, y22) {
    ...
    if (!reverseFlag) {
      ...
      for (let i2 = 0; i2 < len / 2; i2 += 2) {
        let j = len - 2 - i2;
        let tmpX = pointArr[i2];
        let tmpY = pointArr[i2 + 1];
        pointArr[i2] = pointArr[j];
        pointArr[i2 + 1] = pointArr[j + 1];
        pointArr[j] = tmpX;
        pointArr[j + 1] = tmpY;
      }
    }
  ...
  advancedLine(x1, y1, x2, y2, shape) {
    ...
    for (let i2 = 0; i2 < lineLen; i2 += 2) {
      const ox = linePixels[i2];
      const oy = linePixels[i2 + 1];
...
class BrushTool {
  ...
  drawTick(e) {
    ...
    for (let i2 = 0, j = 0; i2 < pixelsWithColors.length; i2 += 3, j += 2) {
      const x2 = pixels[j];
      const y2 = pixels[j + 1];
      const c = player.getColorByCoord(x2, y2);
      pixelsWithColors[i2] = x2;
      pixelsWithColors[i2 + 1] = y2;
      pixelsWithColors[i2 + 2] = c;
    }
  ...
  preRenderBrush() {
    ...
    for (let i2 = 0; i2 < circle.length; i2 += 2) {
      const x2 = circle[i2] + offset;
      const y2 = circle[i2 + 1] + offset;
...
function VirtualListNew($$anchor, $$props) {
  ...
  function findNearestItem(offset) {
    ...
    const i2 = Math.max(0, lastMeasuredIndex);
    if (lastMeasuredSizeAndPosition.offset >= offset) {
      return binarySearch(0, i2, offset);
    } else {
      return exponentialSearch(i2, offset);
    }
  }
...
function Chat($$anchor, $$props) {
  ...
  function chunkArray(arr, size = 6) {
    return Array.from({ length: Math.ceil(arr.length / size) }, (v, i2) => padEnd(arr.slice(i2 * size, i2 * size + size), size, "	"));
  }
      ...
      children: ($$anchor2, $$slotProps) => {
        ...
        var node_8 = child(div_19);
        {
          var consequent_3 = ($$anchor3) => {
            ...
            each(node_9, 19, () => core2.online.users, (user) => user.userId ?? -user.connections[0], ($$anchor4, user, i2) => {
              ...
              var node_10 = first_child(fragment_8);
              {
                ...
                if_block(node_10, ($$render) => {
                  if (get(i2) > 0) $$render(consequent_2);
                });
              }
    ...
    Window($$anchor, spread_props(
      ...
      () => rprops,
      {
        ...
        children: ($$anchor2, $$slotProps) => {
          ...
          each(div, 21, () => tool.bounds, index, ($$anchor3, coordinate, i2) => {
            {
              let $02 = user_derived(() => t(`${pref}.${boundsAlias[i2]}`));
              Legendary($$anchor3, {
                get legend() {
                  return get($02);
                },
                children: ($$anchor4, $$slotProps2) => {
                  Input($$anchor4, {
                    ...
                    draggable: true,
                    get bindValue() {
                      return tool.bounds[i2];
                    },
                    set bindValue($$value) {
                      tool.bounds[i2] = $$value;
                    }
                  });
                }
              });
...
//# sourceMappingURL=2.xDeVVSzl.js.map
